...
exports.course_dets = async (req,res) => {
    ...
    var uid = JSON.parse(ses_data).userid;
    if(uid){
        const qstrng = "select t.year as year,t.course_id as cid,c.title as cname,c.credits as credits from takes as t,course as c where t.course_id = c.course_id and id = \'" + uid + "\' order by year desc;";
        const result = await db_handler.fetchquery(qstrng);
